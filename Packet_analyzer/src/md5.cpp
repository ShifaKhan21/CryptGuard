...
void MD5::update(Context* ctx, const uint8_t* input, uint32_t inputLen) {
    ...
    partLen = 64 - index;
    if (inputLen >= partLen) {
        memcpy((void*)&ctx->buffer[index], (const void*)input, partLen);
        transform(ctx->state, ctx->buffer);
        for (i = partLen; i + 63 < inputLen; i += 64)
            transform(ctx->state, &input[i]);
        index = 0;
    } else i = 0;
    memcpy((void*)&ctx->buffer[index], (const void*)&input[i], inputLen - i);
}
...
void MD5::transform(uint32_t state[4], const uint8_t block[64]) {
    ...
    HH(a, b, c, d, x[ 5], S31, 0xff447b8d); HH(d, a, b, c, x[ 8], S32, 0x66911f03); HH(c, d, a, b, x[11], S33, 0x8be22130); HH(b, c, d, a, x[14], S34, 0x243185be);
    HH(a, b, c, d, x[ 1], S31, 0xfec404eb); HH(d, a, b, c, x[ 4], S32, 0x86353239); HH(c, d, a, b, x[ 7], S33, 0x28db77f5); HH(b, c, d, a, x[10], S34, 0xffeff47d);
    HH(a, b, c, d, x[13], S31, 0x85845dd1); HH(d, a, b, c, x[ 0], S32, 0x6fa87e4f); HH(c, d, a, b, x[ 3], S33, 0xfe2ce6e0); HH(b, c, d, a, x[ 6], S34, 0xa3014314);
    HH(a, b, c, d, x[ 9], S31, 0x4e0811a1); HH(d, a, b, c, x[12], S32, 0xf7537e82); HH(c, d, a, b, x[15], S33, 0xbd3af235); HH(b, c, d, a, x[ 2], S34, 0x2ad7d2bb);
    ...
    II(a, b, c, d, x[ 6], S41, 0x6fa87e4f); II(d, a, b, c, x[13], S42, 0xfe2ce6e0); II(c, d, a, b, x[ 4], S43, 0xa3014314); II(b, c, d, a, x[11], S44, 0x4e0811a1);
    II(a, b, c, d, x[ 2], S41, 0xf7537e82); II(d, a, b, c, x[ 9], S42, 0xbd3af235); II(c, d, a, b, x[15], S43, 0x2ad7d2bb); II(b, c, d, a, x[ 8], S44, 0xeb86d391);
